...
#'
#' @eval recurrent_params("project_directory", "source", "level")
#' @return MSFinder formula and structure data joined together.
import_msfinder_data <- function(project_directory, source, level) {
    msf_formula   <- import_msfinder_formula_file(project_directory, source, level)
    msf_structure <- import_msfinder_structure_file(project_directory, source, level)
...
import_msfinder_formula_file <- function(project_directory, source, level) {
    filename <- get_project_file_path(project_directory, "msfinder_data", source = source,
                                      msfinder_info = "Formula", msfinder_lvl = level)
...
import_msfinder_structure_file <- function(project_directory, source, level) {
    filename <- get_project_file_path(project_directory, "msfinder_data", source = source,
                                      msfinder_info = "Structure", msfinder_lvl = level)
    if (is.na(filename)) return(NULL)
